...
func TestValidate(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "invalidate day: 32 in Jan",
			args:    args{[]string{"32", "01", "1900"}},
			want:    nil,
			wantErr: true,
		},
		{
			name:    "invalidate day: 31 in Sep",
			args:    args{[]string{"31", "09", "1900"}},
			want:    nil,
			wantErr: true,
		},
		{
			name:    "invalidate day: Feb 30th, 2000",
			args:    args{[]string{"30", "2", "2000"}},
			want:    nil,
			wantErr: true,
		},
		{
			name:    "29th day in Feb of 2000",
			args:    args{[]string{"29", "2", "2000"}},
			want:    []int{29, 2, 2000},
			wantErr: false,
		},
		{
...
func Test_isInMonth(t *testing.T) {
	type args struct {
		d int
		m int
		y int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := isInMonth(tt.args.d, tt.args.m, tt.args.y); got != tt.want {
				t.Errorf("isInMonth() = %v, want %v", got, tt.want)
			}
		})
	}
}
